...
const docsRoot = path.join(__dirname, '..', 'docs', 'resources')

for (const entry of RESOURCES) {
  const { slug, name, repo } = entry
  const cePath = path.join(docsRoot, slug, 'common-errors.md')

  // Create or update common-errors.md
  const content = `---
title: ${name} Common Errors & FAQ | FWB Studio Docs
description: Frequently asked questions, troubleshooting, and common error fixes for FiveM ${name} script.
---

# ${name} — Common Errors & FAQ

Have a question or encounter an issue while running **${repo}**? Check the common questions and error solutions below.
...
::: danger Cause
This error occurs because \`fs_bridge\` is missing, not started, or not running on your server before **${repo}** initializes.
:::
...
3. Open \`${repo}/[INSTALL_ME_FIRST]\` and use the item/sql blocks for your inventory system.
...
`
  ...
  fs.mkdirSync(path.dirname(cePath), { recursive: true })
  fs.writeFileSync(cePath, content, 'utf8')
  console.log(`Updated ${slug} -> ${cePath}`)
}

console.log('Finished updating all common-errors.md pages!')
